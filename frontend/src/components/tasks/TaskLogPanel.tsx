    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm font-medium text-[var(--text-primary)]">Real-time execution log</div>
        </div>
        <button
          type="button"
          onClick={copyLogs}
          ...
          {lines.map((line, index) => (
            <div
              ...
              className={`rounded-xl border border-white/5 bg-white/[0.025] px-3 py-2 leading-5 ${
                line.includes('✓') || line.includes('成功') || line.includes('Success') ? 'text-emerald-400' :
                line.includes('✗') || line.includes('失败') || line.includes('错误') || line.includes('Failed') || line.includes('Error') ? 'text-red-400' :
