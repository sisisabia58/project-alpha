...
export default function Proxies() {
  ...
  const metricCards = [
    ...
    { label: 'Successes', value: totalSuccess, icon: Activity, tone: 'text-[var(--accent)]' },
        ...
        <Card className="bg-[var(--bg-pane)]/60">
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-[var(--border-soft)] bg-[var(--bg-pane)]/45 px-3.5 py-2.5 text-xs leading-5 text-[var(--text-secondary)]">
              Supports single proxy entry or multi-line bulk import. Region tags will be saved for filtering and routing.
            </div>
            ...
            {proxies.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8">
                  <div className="empty-state-panel">Proxy pool is empty. Add one or import from the left.</div>
                </td>
              </tr>
            )}
            {proxies.map(p => (
