...
function getCompactStatusMeta(acc: any) {
  ...
  const parts = [
    ...
    `Validity:${getValidityStatus(acc)}`,
  ]
    ...
    try {
      ...
      if (selection.identityProvider === 'mailbox') {
        if (!defaultMailboxProvider?.provider_key) {
          throw new Error('No default email provider configured. Please enable one in Settings first.')
        }
    ...
    <div className="dialog-backdrop" onClick={!taskId ? onClose : undefined}>
      <div className="dialog-panel dialog-panel-md flex flex-col"
        ...
        <div className="px-6 py-4 flex-1 overflow-y-auto flex flex-col gap-5">
          {!taskId ? (
            configLoading ? (
              <div className="text-sm text-[var(--text-muted)]">Loading registration settings...</div>
            ) : (
              <>
                <div>
                  ...
                  <div className="mt-1 text-xs text-[var(--text-muted)]">Whatever the current platform supports will be shown here — no need to study platform capability configs first.</div>
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">Choose Executor Type</div>
                  <div className="mt-1 text-xs text-[var(--text-muted)]">All methods run automatically — they just differ in protocol or browser channel.</div>
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 text-xs text-[var(--text-secondary)]">
                  ...
                  <div className="mt-1">Executor Type: <span className="text-[var(--text-primary)]">{selectedExecutor?.label || '-'}</span></div>
                  <div className="mt-1">Captcha Strategy: <span className="text-[var(--text-primary)]">{getCaptchaStrategyLabel(selection.executorType)}</span></div>
                  {selection.identityProvider === 'oauth_browser' && !reusableBrowser && (
                    <div className="mt-2 text-amber-400">Background browser automation relies on Chrome Profile or Chrome CDP. If not configured, only visible browser automation is allowed.</div>
                  )}
                </div>
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <h2 className="text-base font-semibold text-[var(--text-primary)]">Add Account Manually</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X className="h-4 w-4" /></button>
        </div>
        <div className="px-6 py-4 space-y-3">
          {[['email','Email','text'],['password','Password','text'],['primary_token','Primary Token','text'],['cashier_url','Trial Link','text']].map(([k,l,t]) => (
            <div key={k}>
...
function ActionResultHighlights({ payload }: { payload: any }) {
  ...
  if (payload.remaining_credits) stats.push({ label: 'Remaining Credits', value: payload.remaining_credits })
  if (payload.usage_total) stats.push({ label: 'Used Credits', value: payload.usage_total })
        ...
        <div className="relative overflow-hidden border-b border-[var(--border)] px-6 py-5">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-[var(--text-muted)]">Task status, error summary and real-time log centralized display</p>
            </div>
            <div className="flex items-center gap-2">
              {taskStatus ? (
...
function ActionMenu({
  ...
  const handleActionDone = async (status: string) => {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        if (actionUrl) {
          setToast({ type: 'success', text: data.message || 'Payment link opened in a new tab and copied' })
          return
        }
        ...
        setToast({ type: 'success', text: data.message || 'Action succeeded' })
        return
      }
      setToast({ type: 'success', text: typeof data === 'string' && data ? data : 'Action succeeded' })
    } catch (error: any) {
  ...
  return (
    <div className="relative flex min-w-[136px] items-center justify-end gap-1.5 whitespace-nowrap">
      ...
      {actions.length === 0 && (
        <button
          onClick={() => { if (confirm(`Delete ${acc.email}?`)) apiFetch(`/accounts/${acc.id}`, { method: 'DELETE' }).then(onDelete) }}
    ...
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm flex flex-col" style={{maxHeight:'90vh'}} onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 space-y-3 flex-1 overflow-y-auto min-h-0">
          <div className="relative overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--accent-soft)] p-4 shadow-[var(--shadow-soft)]">
            ...
            <div className="relative flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-right text-[11px] text-[var(--text-muted)] sm:grid-cols-3">
                ...
                <div className="rounded-xl border border-[var(--border-soft)] bg-black/10 px-2.5 py-2">
                  <div className="uppercase tracking-[0.12em]">Plan State</div>
          ...
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Primary Token</label>
...
function ImportModal({ platform, onClose, onDone }: { platform: string; onClose: () => void; onDone: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setResult(`Successfully imported ${res.created} account(s)`); onDone()
      ...
      {open && (
        <div className="absolute right-0 top-10 z-20 min-w-[148px] rounded-lg border border-[var(--border)] bg-[var(--bg-card)] py-1 shadow-lg">
          <div className="px-3 py-1 text-[11px] text-[var(--text-muted)]">
            {hasSelection ? `Export ${selectedIds.length} selected account(s)` : 'Export current filtered results'}
          </div>
          {options.map(option => (
            <button
              key={option.key}
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-2.5 bg-[var(--bg-pane)]/20">
          <div className="flex flex-1 items-center gap-3">
            ...
            >
              ...
              <option value="eligible">Trial Eligible</option>
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              title="Refresh all account credits at once"
              onClick={async () => {
                setBatchRefreshing(true)
                try {
                  const res = await apiFetch(`/accounts/check-all?platform=${tab}`, { method: 'POST' })
                  if (res?.task_id) {
                    setBatchTask({ taskId: res.task_id, title: `Refresh all ${platformLabel} account credits` })
                    setBatchTaskStatus(null)
                  }
                } catch (e) {
                  console.error(e)
            ...
            >
              ...
              {batchRefreshing ? 'Refreshing...' : 'Refresh Credits'}
            </Button>
            ...
            {selectedCount > 0 && (
              <Button
                ...
                onClick={async () => {
                  if (!confirm(`Delete the ${selectedCount} selected account(s)? This action cannot be undone.`)) return
        ...
        <table className="table-fixed w-full min-w-[900px] text-sm">
          ...
          <thead className="sticky top-0 z-10  bg-[var(--bg-pane)]/80">
            <tr className="border-b border-[var(--border)] text-xs uppercase tracking-wider font-medium text-[var(--text-muted)]">
              ...
              <th className="px-3 py-2 text-left">Trial Link</th>
              ...
              <tr>
                <td colSpan={7} className="px-4 py-24 text-center">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    <p className="text-xs text-[var(--text-muted)] max-w-sm">No account records found for the current platform. You can add accounts manually or import them in batch.</p>
                  </div>
                </td>
              </tr>
            )}
