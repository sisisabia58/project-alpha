  ...
  useEffect(() => {
    Promise.all([
      ...
      if (options) {
        ...
        setOptionsError('Provider metadata not loaded. Please restart the backend and refresh the page.')
      }
      setForm(f => {
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_340px]">
        <div className="space-y-4">
            ...
            <CardContent className="space-y-3">
              ...
              {form.identity_provider === 'oauth_browser' && (
                <>
                  ...
                  <Input label="Chrome CDP URL" k="chrome_cdp_url" placeholder="http://localhost:9222" />
                  <p className="text-xs text-[var(--text-muted)]">
                    For third-party accounts using background browser automation, it's recommended to configure Chrome Profile or Chrome CDP to reuse the local logged-in browser session.
                  </p>
                </>
              )}
            </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {mailboxProviderOptions.length > 0 ? (
                  <Select label="Mailbox Service" k="mail_provider" options={mailboxProviderOptions} />
                ) : (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                    No enabled mailbox provider. Please go to settings and add one.
                  </div>
                )}
          ...
          {smsProviderOptions.length > 0 && (
            <Card>
              <CardHeader><CardTitle>SMS Verification</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
                    {optionsError}
                ...
                <CardContent className="space-y-4">
                  ...
                  {task.status === 'interrupted' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">Task was interrupted after server restart</span>
                    </div>
                  )}
                  {task.status === 'cancelled' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">Task was cancelled</span>
                    </div>
                  )}
                </CardContent>
              </Card>
