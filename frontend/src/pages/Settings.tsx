...
const PROVIDER_META: Record<ProviderType, {
  ...
  mailbox: {
    ...
    addDialogHint: 'Choose from the email provider catalog',
    usageHint: 'These email service settings are only used when the registration identity is set to "System Email". You can view details, edit, set as default, or delete directly from each row.',
    ...
    listDescription: (count: number) => `${count} configuration(s). View details, edit, set default, or delete.`,
    noAvailableText: 'No email providers are currently available to add',
    availableText: (count: number) => `${count} more email provider(s) available to add`,
    emptyText: 'No email provider configured. Please add one first.',
  ...
  captcha: {
    ...
    addDialogHint: 'Choose from the captcha provider catalog',
    usageHint: 'In protocol mode, enabled remote captcha services are auto-selected in order; in browser mode, the current default captcha provider is used. You can view details, edit, set as default, or delete directly from each row.',
    ...
    listDescription: (count: number) => `${count} configuration(s). Protocol mode reads available entries here in order.`,
    noAvailableText: 'No captcha providers are currently available to add',
    availableText: (count: number) => `${count} more captcha provider(s) available to add`,
    emptyText: 'No captcha provider configured. Please add one first.',
  ...
  sms: {
    ...
    addDialogHint: 'Choose from the SMS provider catalog',
    usageHint: 'When the platform requires phone verification, enabled SMS providers here will create temporary numbers and fill in the SMS code. You can view details, edit, set as default, or delete directly from each row.',
    ...
    listDescription: (count: number) => `${count} configuration(s). Phone verification and SMS checks prefer the default entry here.`,
    noAvailableText: 'No SMS providers are currently available to add',
    availableText: (count: number) => `${count} more SMS provider(s) available to add`,
    emptyText: 'No SMS provider configured. Please add one first.',
        ...
        return (
          <div key={p.name} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                <p className="text-xs text-[var(--text-muted)] mb-2">Third-party Login</p>
                <div className="flex flex-wrap gap-4">
...
const TABS: { id: string; label: string; icon: any; sections?: any[] }[] = [
  {
    id: 'register', label: 'Registration Policy', icon: Cpu,
    sections: [{
      section: 'Default Registration Policy',
      desc: 'These are the default behaviors. The account list and registration page will reuse these settings.',
      items: [
        { key: 'default_identity_provider', label: 'Default Registration Identity' },
        { key: 'default_oauth_provider', label: 'Default Third-party Login', placeholder: '' },
        { key: 'default_executor', label: 'Default Executor Type' },
      ],
    }, {
      section: 'Browser Reuse',
      desc: 'For third-party accounts using background browser automation, you usually need to reuse an already-logged-in browser on this machine.',
      items: [
        ...
        { key: 'chrome_cdp_url', label: 'Chrome CDP URL', placeholder: 'http://localhost:9222' },
      ],
    }],
  },
  {
  ...
  },
  {
    ...
    sections: [{
      ...
      section: 'Any2Api',
      desc: 'Sync accounts to the Any2Api service for export and integration',
      items: [
  ...
  const queryPrice = async () => {
    ...
    try {
      ...
      if (current) {
        ...
        setMessage('No price info found for the current service/country')
      }
    } catch (e: any) {
    ...
    <div className="rounded-xl border border-sky-500/20 bg-sky-500/10 px-3 py-3 text-xs text-[var(--text-secondary)]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-[var(--text-muted)]">Query balance/price using the current API Key, service code and country ID.</div>
        </div>
        ...
        <div className="px-6 py-4">
          {providers.length === 0 ? (
            <div className="empty-state-panel">
              All providers currently available to add are already in the list.
            </div>
          ) : (
            <div className="space-y-3">
      ...
      <div className="dialog-panel dialog-panel-md flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          {currentFields.length === 0 ? (
            <div className="text-sm text-[var(--text-muted)] py-3">This driver family requires no additional fields.</div>
          ) : (
            <GroupedProviderFields
...
export default function Settings({ embedded, defaultTab }: { embedded?: boolean; defaultTab?: string }) {
  ...
  const getSelectOptions = (key: string) => {
    ...
    if (key === 'default_oauth_provider') {
      return [
        { label: 'No preset — chosen by current page', value: '' },
        ...
        <div className="space-y-4">
          {activeTab === 'platform_caps' ? (
            ...
            <>
              {activeTab === 'register' && (
                <div className="rounded-lg border border-[var(--accent-edge)] bg-[var(--accent-soft)] px-4 py-3 text-sm text-[var(--text-secondary)]">
                  Regular users only need to understand two things: choose "System Email" or "Third-party Account" as the registration identity, and choose "Protocol Mode / Background Browser Automation / Visible Browser Automation" as the executor. These settings only set the defaults.
                </div>
              )}
